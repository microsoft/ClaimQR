...
program.requiredOption('-q, --qrPath <qrPath>', 'path to the input QR code text or image');
...
void (async () => {
    try {
        await verifyQrFiles(options.type, options.qrPath, options.outJwtPath, options.jwksPath);
    } catch (err) {
        console.log(err);
    }
})();